...
node_join() {
    ...
    if [ -v AIRGAP ]; then
        ssh -i ~/.ssh/id_shared $addr "$params INSTALL_RKE2_ARTIFACT_PATH=/root/rke2-artifacts sh /root/install.sh"
    else
        ssh -i ~/.ssh/id_shared $addr "$params INSTALL_RKE2_VERSION=$RKE2_VERSION sh /root/install.sh"
    fi

    # Start rke2 service
    ssh -i ~/.ssh/id_shared $addr "systemctl enable --now rke2-${type/init/server}"
    ssh -i ~/.ssh/id_shared $addr "journalctl -u rke2-${type/init/server} | grep 'Started Rancher Kubernetes Engine v2'"

    if [ $type == init ]; then
        INIT_SERVER=$addr
        INIT_TOKEN=$(ssh -i ~/.ssh/id_shared $addr cat /var/lib/rancher/rke2/server/node-token)
        scp -i ~/.ssh/id_shared $addr:/etc/rancher/rke2/rke2.yaml "$WORKDIR/admin.conf"
